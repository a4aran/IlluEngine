...
UI is a class that combines 2 sub-classes [GUI*](?load=gui) and [HUD](#).
...
[GUI*](?load=gui) and [HUD](#) are privated so the only way to create objects of thier sub-classes is to use **creator methods** (they start with `new`).
