...
UI is a class that combines 2 sub-classes [GUI*](?load=gui.md) and [HUD](#).
...
[GUI*](?load=gui.md) and [HUD](#) are privated so the only way to create objects of thier sub-classes is to use **creator methods** (they start with `new`).
